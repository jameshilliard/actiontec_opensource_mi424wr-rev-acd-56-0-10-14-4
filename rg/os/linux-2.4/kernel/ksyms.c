...
unsigned int ing_hook(unsigned int hook, struct sk_buff **pskb,
...
#undef kmalloc
#undef kfree
EXPORT_SYMBOL(kmem_cache_alloc_debug);
EXPORT_SYMBOL(kmem_cache_free_debug);
EXPORT_SYMBOL(kmalloc_debug);
EXPORT_SYMBOL(kfree_debug);
#endif
...
EXPORT_SYMBOL(kmem_cache_has_free_space);
EXPORT_SYMBOL(kmem_get_free_ram_size);
...
/* QoS ingress hook */
EXPORT_SYMBOL(ing_hook);
#endif

/* DSCP to priority translation array */

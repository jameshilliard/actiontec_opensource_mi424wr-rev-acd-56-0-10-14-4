...
//we use offset > OFFSET_OP as an OP
//we think normally it's impossible that offset will be more than 0xa0
#define OFFSET_OP 0xA0
enum U32_OP {
    AND_OP=160,
    OR_OP=164,
    LE_OP=168,
...
static int u32_classify(struct sk_buff *skb, struct tcf_proto *tp, struct tcf_result *res)
{
	...
	int sel = 0;
	int i;
//printk ("**********8u32 classify t=%08x\n", tp);
next_ht:
	n = ht->ht[sel];
//printk ("----n->sel.nkeys=%d, %08x, off2=%d\n", n->sel.nkeys, n, off2);
next_knode:
	if (n) {
        ...
        i = n->sel.nkeys; 
        while (i > 0 ) {
            if (key->off >= OFFSET_OP) { //is an OP, not a real match
//                    op = key->off; //(key->off & 0x0f) >> 2;
                //printk ("Got op =%d, off=%4x\n", op, key->off);
                switch (key->off) {
                case AND_OP:
                    curr_op[++idx_op] = AND_OP;
                    isrange = 0;
                    break;
                case OR_OP:
                    isrange = 0;
                    curr_op[++idx_op] = OR_OP;
                    break;
                case END_OP:
                    isrange = 0;
                    idx_op--;
                ...
                if (!isrange) { //normal case AND
                    matched = !((*(u32*)(ptr+key->off+(off2&key->offmask))^key->val)&key->mask);
                }
                else if (isrange == LE_OP) {
                    matched = (*(u32*)(ptr+key->off+(off2&key->offmask)) &key->mask)<= key->val;
//                        printk ("LE: %08x, %08x, %d, match=%d\n", 
//                              (*(u32*)(ptr+key->off+(off2&key->offmask)) &key->mask), key->val, key->val,
//                            matched);
                }
                else {
                    matched = (*(u32*)(ptr+key->off+(off2&key->offmask)) &key->mask) >= key->val;
  //                  printk ("LG: %08x, %08x, %d, matched=%d\n", 
    //                        (*(u32*)(ptr+key->off+(off2&key->offmask)) &key->mask), key->val, key->val, matched);
                }
                isrange = 0;
            }
check_result:
            if (!idx_op) {//we are at the top layer
                if (curr_op[idx_op] == AND_OP) {
                    if (!matched) {//not match this rule , go to check next rule
                        n = n->next;
                        goto next_knode;
                    }
                }
                else if (matched) { //or OP, this rule matched
                    break;
                }
            }
            else {//we are still in the stack , not on the top
                if (((curr_op[idx_op] == AND_OP) && !matched) || 
                    ((curr_op[idx_op] == OR_OP) && matched)) {
                    int need_match_endop = 1;

                    idx_op--;
                    //skip all other same level conditions till END_OP
                    do {
                        key++;
                        i--;
                        if (key->off >= OFFSET_OP) {
                            switch (key->off) {
                            case END_OP:
                                need_match_endop--;
                                break;
                    ...
                    } while (need_match_endop && (i > 0));
                }
            }
            i--;
            key++;
        }
        if (!matched) { //we may reach i==0, level 0 is AND_OP
            n = n->next;
            goto next_knode;
        }
#else
		      ...
		      unsigned long *arg)
{
    ...
    if (n) {
        ...
        int i;
        for (i = n->sel.nkeys; i>0; i--, key++) {
            printk ("i=%d, off=0x%02x, offmask=0x%02x, val=0x%04x, mask=0x%04x\n", 
                    i, key->off, key->offmask, key->val, key->mask );
        }
    }
    //end of jzj
#endif // debug

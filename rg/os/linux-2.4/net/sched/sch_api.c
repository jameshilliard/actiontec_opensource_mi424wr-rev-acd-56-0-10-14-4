...
static int sch_newdefaults(struct sk_buff *skb, struct nlmsghdr *n, void *arg)
{
	struct rtattr **tca = arg;
	struct rtattr *kind = tca[TCA_KIND-1];
	...
	if (!rtattr_strcmp(kind, "prio2band"))
	{
		struct tc_prio_qopt *qopt = RTA_DATA(opt);

		memcpy(prio2band, qopt->priomap, TC_PRIO_MAX+1);
	}
	else if (!rtattr_strcmp(kind, "dscp2prio"))
		memcpy(ip_dscp2prio, RTA_DATA(opt), sizeof(ip_dscp2prio));
	else
		return -EINVAL;
...
int __init pktsched_init(void)
{
	...
	if (link_p) {
		...
		link_p[RTM_NEWDEFAULTS-RTM_BASE].doit = sch_newdefaults;
	}

...
#ifndef CONFIG_RG_QOS_INGRESS_FEATURE
...
//	printk ("-------ingress result %d class 0x%04x, ds=%04x, cmp=%d,%d\n", result, res.classid,
 //           ipv4_get_dsfield(skb->nh.iph), res.classid == 0x50001, 
   //         (res.classid & 0x0fff) == 0x50001);
	/*
...
#ifdef ACTION_TEC_DIFFSERV
            {
            //AF11 - 0x50001, AF12 - 0x50002, AF13 - 0x50003
            //AF21 - 0x60001, AF22 - 0x60002, AF23 - 0x60003
            //...
            //BF   - 0x90001  EF 0x90002
            __u8 value;
            __u8 major_AF, minor_AF;

            major_AF = (res.classid >> 16) - 4 ;
            minor_AF = res.classid & 0xff;
            if (major_AF <=5  && minor_AF <= 3 && major_AF >=1) {
                value = 0;
                if (5 == major_AF) { //EF
                    value = (minor_AF == 2)?0xb8:0;
                }
                else {
                    value = (major_AF << 5) + (minor_AF << 3);
                }

//                printk ("-----value=%02x, cls=%04x, major=%d,%02x minor=%d\n", 
//                        value, res.classid, major_AF, minor_AF);
                switch (skb->protocol) {
...
printk ("ingress tc_index from %08x->%08xn", 
...
int ingress_init(struct Qdisc *sch,struct rtattr *opt)
{
	...
	/* overriding the nf hooks with knet_hooks.
	 * if OpenRG's QoS is defined - we want to go through the ingress qdisc
	 * before other knet hooks might catch packets and hadle them */
#ifndef CONFIG_RG_QOS_INGRESS_FEATURE
	if (!nf_registered) {

...
extern int (*vlan_rx_hook)(struct sk_buff **skb, struct net_device *dev);
extern int (*vlan_tx_hook)(struct sk_buff **skb, struct net_device *dev);
...
#ifdef CONFIG_VLAN_8021Q_FAST
	vlan_rx_hook = __vlan_rx_hook;
	vlan_tx_hook = __vlan_tx_hook;
#else
...
struct vlan_group *__vlan_find_group(int real_dev_ifindex)
{

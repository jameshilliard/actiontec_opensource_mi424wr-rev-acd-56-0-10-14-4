...
#else
int __vlan_rx_hook(struct sk_buff **skb, struct net_device *dev)
{
	unsigned char *rawp = NULL;
	struct sk_buff *__skb = *skb;
	struct vlan_hdr *vhdr = (struct vlan_hdr *)(__skb->data);
	unsigned short vid;
	struct net_device_stats *stats;
	unsigned short vlan_TCI;
	unsigned short proto;
	...
	if (__skb->protocol != __constant_htons(ETH_P_8021Q) || 
		!__vlan_find_group(dev->ifindex))
	{
		/* This is not a VLAN type packet or not an underlying device
		 * of a virtual VLAN device */
		return 0;
	}
	...
	__skb->dev = __find_vlan_dev(dev, vid);
	if (!__skb->dev) {
		spin_unlock_bh(&vlan_group_lock);
	...
	__skb->dev->last_rx = jiffies;

	/* Bump the rx counters for the VLAN device. */
	stats = vlan_dev_get_stats(__skb->dev);
	stats->rx_packets++;
	stats->rx_bytes += __skb->len;

	skb_pull(__skb, VLAN_HLEN); /* take off the VLAN header (4 bytes currently) */

	/* Ok, lets check to make sure the device (dev) we
	 * came in on is what this VLAN is attached to.
	 */

	if (dev != VLAN_DEV_INFO(__skb->dev)->real_dev) {
		spin_unlock_bh(&vlan_group_lock);

#ifdef VLAN_DEBUG
		printk(VLAN_DBG __FUNCTION__ ": dropping skb: %p because came in on wrong device, dev: %s  real_dev: %s, skb_dev: %s\n",
		       __skb, dev->name, 
		       VLAN_DEV_INFO(__skb->dev)->real_dev->name,
		       __skb->dev->name);
#endif
		kfree_skb(__skb);
		stats->rx_errors++;
	 ...
	 */
	__skb->priority =
		vlan_get_ingress_priority(__skb->dev, ntohs(vhdr->h_vlan_TCI));

#ifdef VLAN_DEBUG
	printk(VLAN_DBG __FUNCTION__ ": priority: %lu  for TCI: %hu (hbo)\n",
	       (unsigned long)(__skb->priority), ntohs(vhdr->h_vlan_TCI));
#endif

	/* The ethernet driver already did the pkt_type calculations
	 * for us...
	 */
	switch (__skb->pkt_type) {
	case PACKET_BROADCAST: /* Yeah, stats collect these together.. */
		// stats->broadcast ++; // no such counter :-(
		break;
	...
	case PACKET_OTHERHOST: 
		 ...
		 * device, and still route correctly.
		 */
		if (memcmp(__skb->mac.ethernet->h_dest,
			__skb->dev->dev_addr, ETH_ALEN) == 0) 
		{
			/* It is for our (changed) MAC-address! */
			__skb->pkt_type = PACKET_HOST;
		}
		break;
	default:
		break;
	...
	__skb->protocol = proto;
	if (ntohs(proto) >= 1536) {
		/* let it continue to be handled by true layer 3 protocols.
		 */
	    	goto Finish;
	}
	 ...
	 */
	if (*(unsigned short *)rawp == 0xFFFF) {
		__skb->protocol = __constant_htons(ETH_P_802_3);
		/* let it continue to be handled by true layer 3 protocols.
		 */
		goto Finish;
	}
	 ...
	 *	Real 802.2 LLC
	 */
	__skb->protocol = __constant_htons(ETH_P_802_2);
	/* place it back on the queue to be handled by upper layer protocols.
	 */
...
Finish:
	 ...
	 * to make it look like ethernet...
	 */
	*skb = vlan_check_reorder_header(__skb);
	__skb = *skb;
...
int __vlan_tx_hook(struct sk_buff **skb, struct net_device *dev)
{
	struct net_device_stats *stats;
	struct sk_buff *__skb = *skb;
	struct vlan_ethhdr *veth = (struct vlan_ethhdr *)(__skb->data);

	if (!(dev->priv_flags & IFF_802_1Q_VLAN))
	{
		/* Not a VLAN device - nothing to do */
		return 0;
	}

	stats = vlan_dev_get_stats(dev);
	...
	if (veth->h_vlan_proto != __constant_htons(ETH_P_8021Q)) {
		...
		if (skb_headroom(__skb) < VLAN_HLEN) {
			*skb = skb_realloc_headroom(__skb, VLAN_HLEN);
			kfree_skb(__skb);
			if (*skb == NULL) {
				stats->tx_dropped++;
				return -ENOMEM;
			...
			if (!(*skb = skb_unshare(__skb, GFP_ATOMIC))) {
				printk(KERN_ERR "vlan: failed to unshare skbuff\n");
				stats->tx_dropped++;
				return -ENOMEM;
			}
		...
		veth = (struct vlan_ethhdr *)skb_push(__skb, VLAN_HLEN);

		/* Move the mac addresses to the beginning of the new header. */
		memmove(__skb->data, __skb->data + VLAN_HLEN, 12);
		 ...
		 */
		veth_TCI = VLAN_DEV_INFO(dev)->vlan_id;
		veth_TCI |= vlan_dev_get_egress_qos_mask(dev, __skb);

		veth->h_vlan_TCI = htons(veth_TCI);
	}

	__skb->dev = VLAN_DEV_INFO(dev)->real_dev;

#ifdef VLAN_DEBUG
	printk(VLAN_DBG __FUNCTION__ ": about to send skb: %p to dev: %s\n",
	       __skb, __skb->dev->name);
	printk(VLAN_DBG "  %2hx.%2hx.%2hx.%2xh.%2hx.%2hx %2hx.%2hx.%2hx.%2hx.%2hx.%2hx %4hx %4hx %4hx\n",
	       veth->h_dest[0], veth->h_dest[1], veth->h_dest[2], veth->h_dest[3], veth->h_dest[4], veth->h_dest[5],
	       veth->h_source[0], veth->h_source[1], veth->h_source[2], veth->h_source[3], veth->h_source[4], veth->h_source[5],
	       veth->h_vlan_proto, veth->h_vlan_TCI, veth->h_vlan_encapsulated_proto);

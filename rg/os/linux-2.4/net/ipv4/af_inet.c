 ...
 *
 * Version:	$Id: af_inet.c,v 1.3 2007/10/24 23:28:29 athill Exp $
 *
...
#ifdef CONFIG_IPSEC
int (*ipsec_ioctl_hook)(unsigned long arg);
#endif
...
static int inet_ioctl(struct socket *sock, unsigned int cmd, unsigned long arg)
{
	...
	switch(cmd) {
		...
		case SIOCSIPSECDEV:
#ifdef CONFIG_IPSEC
			if (ipsec_ioctl_hook != NULL)
				return ipsec_ioctl_hook(arg);
#endif
			return -ENOPKG;
		...
		/*
 		 * Multicast source filter ioctls
		 */
		case SIOCSIPMSFILTER:
			return in_setmcast_srcfilter(sk, arg);

		case SIOCGIPMSFILTER:
			return in_getmcast_srcfilter(sk, arg);

		case SIOCSMSFILTER:
		case SIOCGMSFILTER:
			/*
			 * Protocol independent ioctls not supported, yet.
			 */
			return -EOPNOTSUPP;

 ...
 *		The IP to API glue.
 *		
 * Version:	$Id: ip_sockglue.c,v 1.3 2007/10/24 23:28:29 athill Exp $
 *
 * Authors:	see ip.c
 *
 * Fixes:
...
int ip_setsockopt(struct sock *sk, int level, int optname, char *optval, int optlen)
{
	...
	if (optname >= IGMP_PROXY_BASE && optname <= IGMP_PROXY_END)
		return igmprx_setsockopt(sk, optname, optval, optlen);
#endif
#ifdef CONFIG_RG_IGMP_PROXY_MODULE
	if (igmp_proxy_setsockopt &&
		optname >= IGMP_PROXY_BASE && optname <= IGMP_PROXY_END)
	{
	...
	switch (optname) {
		...
		case MCAST_LEAVE_SOURCE_GROUP:
		 	err = ip_setmoptions(sk, optname, optval, optlen);
			break;
#else

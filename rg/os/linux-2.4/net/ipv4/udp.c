 ...
 *
 * Version:	$Id: udp.c,v 1.4 2008/04/09 23:44:34 athill Exp $
 *
...
/*
 * Check source and destination addresses of an incoming datagram against the
 * multicast source filter list for a socket.  Return 1 if the datagram should
 * be blocked, 0 otherwise.
 */
static inline int udp_v4_mcast_block(struct sock *sk, u32 saddr, u32 daddr)
{
	struct sock_mcastsf *smsf;
	struct source_list *psrc;
	int i;
	int block = 1;

	//printk(KERN_DEBUG "udp_v4_mcast_block\n");

	for (i = 0; i < sk->protinfo.af_inet.mc_num_memberships; i++) {
		if (sk->protinfo.af_inet.mc_membership[i]->inm_addr.s_addr 
		    == daddr) {
			smsf = &sk->protinfo.af_inet.mc_membershipsf[i];
			psrc = smsf->smsf_slist;
			while (psrc) {
				if (saddr < psrc->src_addr.s_addr) {
					psrc = psrc->src_next;
					continue;
				}
				if (saddr != psrc->src_addr.s_addr) {
			...
			if (smsf->smsf_fmode == MCAST_INCLUDE) {
				block = (psrc == NULL);
			} else {
				block = (psrc != NULL);
			}
					     ...
					     int dif)
{
	...
	for(; s; s = s->next) {
		if ((s->num != hnum)					||
		    ...
		    udp_v4_mcast_block(s, rmt_addr, loc_addr))
#else 
		    (s->bound_dev_if && s->bound_dev_if != dif))
#endif /* ACTION_TEC_IGMPV3 */

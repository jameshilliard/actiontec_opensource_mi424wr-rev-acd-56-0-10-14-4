...
static int (*net_rx_hook)(struct sk_buff **skb);
static int (*net_tx_hook)(struct sk_buff **skb);
...
int dev_queue_xmit(struct sk_buff *skb)
{
	...
	if (net_tx_hook && !net_tx_hook(&skb))
	{
...
static void get_sample_stats(int cpu)
{
	...
	int rq;
#endif
	int blog = softnet_data[cpu].input_pkt_qlen;
...
int netif_rx(struct sk_buff *skb)
{
	...
	if (skb->protocol == htons(ETH_P_8021Q))
	{
		skb->priority = vlan_get_ingress_priority(skb->dev,
		    ntohs(((struct vlan_hdr *)skb->data)->h_vlan_TCI));
	}
	...
	band = skb_prio2band(skb->priority);

	local_irq_save(flags);

	netdev_rx_stat[this_cpu].total++;
	if (queue->input_pkt_qlen <= netdev_max_backlog) {
		if (queue->input_pkt_qlen) {
			if (queue->throttle)
				goto drop;

enqueue:
			dev_hold(skb->dev);
			__skb_queue_tail(&queue->input_pkt_queue[band],skb);
			queue->input_pkt_qlen++;
...
int netif_receive_skb(struct sk_buff *skb)
{
	...
	if (net_rx_hook && !net_rx_hook(&skb))
		return NET_RX_SUCCESS;
...
static int process_backlog(struct net_device *blog_dev, int *budget)
{
	...
	for (;;) {
		...
		local_irq_disable();
		/* take the first packet at the highest priority queue */
		for (rq=0; rq<NUM_RECV_QUEUES; rq++) {
			if ((skb = __skb_dequeue(&queue->input_pkt_queue[rq]))) {
				queue->input_pkt_qlen--;
				break;
			}
		}
...
static int dev_get_next(int *if_index)
{
	struct net_device *dev;
	int dev_idx, min_larger = 0;

	for (dev = dev_base; dev; dev = dev->next) {
	        if (!(dev->flags & IFF_UP))
		    continue;
		dev_idx = dev->ifindex;
		if (dev_idx > *if_index && (!min_larger ||
		    dev_idx < min_larger)) {
			min_larger = dev_idx;
		}
	}
...
int dev_ioctl(unsigned int cmd, void *arg)
{
	...
	switch(cmd) 
	{
		...
		case SIOCGIFNEXT:
			if (dev_get_next(&ifr.ifr_ifindex))
			    return -EINVAL;
...
int __init net_dev_init(void)
{
	...
	for (i = 0; i < NR_CPUS; i++) {
		...
		for (rq=0; rq<NUM_RECV_QUEUES; rq++)
			skb_queue_head_init(&queue->input_pkt_queue[rq]);
		queue->input_pkt_qlen = 0;
		queue->throttle = 0;
		queue->cng_level = 0;

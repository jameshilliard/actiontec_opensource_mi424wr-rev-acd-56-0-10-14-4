...
#define REACHABLE_EXPIRES_FACTOR 10
				...
				struct net_device *dev)
{
	...
	n->real_confirmed = n->confirmed = jiffies - (n->parms->base_reachable_time<<1);
...
static void neigh_timer_handler(unsigned long arg) 
{
	...
	NEIGH_PRINTK3(KERN_DEBUG "neigh_timer_handler(neigh=%p) state=%d j=%d\n", neigh,
	...
	if (state&NUD_REACHABLE) {
		NEIGH_PRINTK3(KERN_DEBUG "neigh_timer_handler(neigh=%p) REACHABLE now=%d, used=%d "
			"confirmed=%d rt=%d\n", neigh, now, neigh->used, neigh->confirmed,
		...
		NEIGH_PRINTK3(KERN_DEBUG "neigh %p: DELAY now=%d conf=%d rt=%d\n", neigh, now,
		...
		/* a neighbour is considered reachable for
		 * reachable_time * REACHABLE_EXPIRES_FACTOR jiffies. we do that by
		 * fooling the state machine by setting confirmed to now automatically
		 * in the probe state if now - real_confirmed (the time in which we
		 * really got an arp reply) is less than (reachable_time *
		 * REACHABLE_EXPIRES_FACTOR).
		 * the state machine works regularly -
		 * REACHABLE -> DELAY,
		 * DELAY -> PROBE (and send an arp request!, note that it
		 *     doesn't enter here, because its an 'else if')
		 * PROBE -> REACHABLE (this patch) if less than the time above.
		 * if the time above passes without an ARP reply, PROBE falls through to
		 * the 'if' below where it changes to FAILED.
		 */
		NEIGH_PRINTK3(KERN_DEBUG "neigh %p: PROBE now=%d conf=%d rc=%d rt=%d\n", neigh, now,
			neigh->confirmed, neigh->real_confirmed, neigh->parms->reachable_time);
		if (now - neigh->real_confirmed <
			neigh->parms->reachable_time * REACHABLE_EXPIRES_FACTOR) {
			/* We got a timeout on the probe, fake it. */
			NEIGH_PRINTK3(KERN_DEBUG "neigh %p: PROBE->REACHABLE (neigh_timer_handler).\n",
				neigh);
			neigh->confirmed = now;
			neigh->nud_state = NUD_REACHABLE;
			neigh_connect(neigh);
	...
	if (neigh->nud_state&NUD_IN_TIMER) {
		...
		if (neigh->nud_state&(NUD_INCOMPLETE|NUD_PROBE)) {
			write_unlock_bh(&neigh->lock);
			NEIGH_PRINTK3(KERN_DEBUG "neigh_timer_handler(neigh=%p): solicit\n", neigh);
...
int __neigh_update(struct neighbour *neigh, const u8 *lladdr, u8 new, int flags, int arp)
{
	...
	if (new&NUD_CONNECTED)
		neigh->real_confirmed = neigh->confirmed = now;
	if (lladdr != neigh->ha) {
		neigh->updated = now;
		memcpy(&neigh->ha, lladdr, dev->addr_len);
		neigh_update_hhs(neigh);
		if (!(new&NUD_CONNECTED))
			neigh->real_confirmed = neigh->confirmed =
				now - (neigh->parms->base_reachable_time<<1);
		notify = 1;
	}
	if (new == old)
		goto out;

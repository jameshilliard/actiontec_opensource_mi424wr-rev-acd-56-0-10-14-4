...
void dump_stack_backtrace_instr(struct task_struct *tsk, struct pt_regs *regs)
{		
	printk("Process %s (pid: %d, stack limit = 0x%p)\n",
		tsk->comm, tsk->pid, tsk + 1);

    	if (!user_mode(regs) || in_interrupt()) {
		I_really_mean_dump_stack_so_dont_mess_with_me(tsk, 
...
void dump_all(struct task_struct *tsk, struct pt_regs *regs)
{		
	show_regs(regs);
	dump_stack_backtrace_instr(tsk, regs);
}
...
asmlinkage void do_undefinstr(int address, struct pt_regs *regs, int mode)
{
	...
	printk(KERN_ERR "%s (%d): undefined instruction: pc=%p\n",
...
#ifdef CONFIG_DEBUG_USER
	printk(KERN_ERR "%s (%d): address exception: pc=%08lx\n",

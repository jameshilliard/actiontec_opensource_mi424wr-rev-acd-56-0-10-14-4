...
#define PCI1520_SYSCTL_REG	0x80
#define PCI1520_MFROUT_REG	0x8c
#define PCI1520_DEVCTL_REG	0x92
#define INTRTIE			BIT(29)
#define P2CCLK  		BIT(27)
#define MRBURSTUP		BIT(14)
#define INTA2FUNC0		BIT(1)
#define PARPCIONLY_MASK		(BIT(1)|BIT(2))


static void __init pci_fixup_final_ti1520(struct pci_dev *dev)
{
	u32 val;
	
	if (!PCI_FUNC(dev->devfn))
	{
		pci_read_config_dword(dev, PCI1520_SYSCTL_REG, &val);
		/* INTA + INTB -> INTA, ext. clock to power switch, 
		 * enable upstream memory read burst */
    		val |= INTRTIE | P2CCLK | MRBURSTUP;
    		pci_write_config_dword(dev, PCI1520_SYSCTL_REG, val);
	
		pci_read_config_dword(dev, PCI1520_DEVCTL_REG, &val);
		/* parallel PCI interrupts only */
    		val &= ~PARPCIONLY_MASK;
    		pci_write_config_dword(dev, PCI1520_DEVCTL_REG, val);

		/* map INTA to MFUNC0 */
		pci_write_config_dword(dev, PCI1520_MFROUT_REG, INTA2FUNC0);
	}
}
#endif
...
extern struct hw_pci sl2312_pci;
extern struct hw_pci ixdp425_pci;
extern struct hw_pci matecumbe_pci;
extern struct hw_pci napa_pci;
extern struct hw_pci yosemite_pci;
extern struct hw_pci coyote_pci;
extern struct hw_pci hg21_pci;
extern struct hw_pci bamboo_pci;
extern struct hw_pci jeeves_pci;
extern struct hw_pci wx5800_pci;
extern struct hw_pci wx5711_pci;
extern struct hw_pci wx5715_pci;
extern struct hw_pci wagp100g_pci;
extern struct hw_pci kingscanyon_pci;
extern struct hw_pci rockawaybeach_pci;
extern struct hw_pci cx8620x_pci;
extern struct hw_pci montejade_pci;
extern struct hw_pci bruce_pci;
extern struct hw_pci mi424wr_pci;
extern struct hw_pci vi414wg_pci;
extern struct hw_pci ri408_pci;
...
void __init pcibios_init(void)
{
	...
	do {
		...
		if (machine_is_yosemite()) {
			hw = &yosemite_pci;
			break;
		}
		if (machine_is_coyote()) {
			hw = &coyote_pci;
			break;
		}
		if (machine_is_hg21()) {
		...
		if (machine_is_bamboo()) {
			hw = &bamboo_pci;
			break;
		}
		if (machine_is_jeeves()) {
			hw = &jeeves_pci;
			break;
		}

		/* XXX: Workaround for compiler bug: No more than 10
		 * if (0) statements in do-while(0) */
		do {
		if (machine_is_gtwx5800()) {
			hw = &wx5800_pci;
			break;
		}
		if (machine_is_gtwx5711()) {
			hw = &wx5711_pci;
			break;
		}
		...
		if (machine_is_wav54g()) {
			hw = &wagp100g_pci;
			break;
		}
		if (machine_is_kingscanyon()) {
			hw = &kingscanyon_pci;
			break;
		}
		if (machine_is_rockawaybeach()) {
			hw = &rockawaybeach_pci;
			break;
		}
		if (machine_is_montejade()) {
			hw = &montejade_pci;
			break;
		}
		if (machine_is_bruce()) {
...
#endif
#ifdef CONFIG_ARCH_CX8620X
		if (machine_is_cx8620x()) {
			hw = &cx8620x_pci;
			break;
		}
#endif
	 ...
	 * TODO: MAKE THIS DIE
	 */
#if !defined(CONFIG_ARCH_IXP1200) && !defined(CONFIG_ARCH_IQ80310) && \
	!defined(CONFIG_ARCH_BRH) && !defined(CONFIG_ARCH_IXP425) && \
	!defined(CONFIG_ARCH_CX8620X) && !defined(CONFIG_ARCH_SL2312)

/****************************************************************************
 *
 * /home/bat/bat/actiontec_bhr_4_0/rg/os/linux-2.4/arch/arm/mach-ixp425/rockawaybeach-pci.c
 * 
...
#define IXP425_PCI_INTA_IRQ	IRQ_IXP425_GPIO4
#define IXP425_PCI_INTB_IRQ	IRQ_IXP425_GPIO5

/* Slots configuration */
#define ROCKAWAYBEACH_MINI_PCI_0	2
#define ROCKAWAYBEACH_MINI_PCI_1	1

#ifdef CONFIG_PCI_RESET

#define IXP425_PCI_INTA_GPIO	IXP425_GPIO_PIN_4
#define IXP425_PCI_INTB_GPIO	IXP425_GPIO_PIN_5

/* PCI controller pin mappings */
#define IXP425_PCI_RESET_GPIO	IXP425_GPIO_PIN_0
...
void __init rockawaybeach_pci_hw_init(void)
{
	/* Disable PCI clock */
...
void __init rockawaybeach_pci_init(void *sysdata)
{
#ifdef CONFIG_PCI_RESET
	if (ixp425_pci_is_host())
		rockawaybeach_pci_hw_init();
#endif
	ixp425_pci_init(sysdata);
}

static int __init rockawaybeach_map_irq(struct pci_dev *dev, u8 slot, u8 pin)
{
	int res = -1;
	if ((pin < 1) || (4 < pin))
		goto Exit;
	...
	switch(slot)
	{
	case ROCKAWAYBEACH_MINI_PCI_0:
		res = IXP425_PCI_INTA_IRQ;
		break;

	case ROCKAWAYBEACH_MINI_PCI_1:
		res = IXP425_PCI_INTB_IRQ;
		break;
	}
...
struct hw_pci rockawaybeach_pci __initdata = {
    init:	rockawaybeach_pci_init,
    swizzle:	common_swizzle,
    map_irq:	rockawaybeach_map_irq,
};

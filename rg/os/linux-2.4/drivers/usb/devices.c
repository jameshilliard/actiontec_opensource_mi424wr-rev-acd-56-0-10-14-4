 ...
 *
 * $Id: devices.c,v 1.1.1.1 2006/01/20 03:16:11 zjjiang Exp $
 */
...
static char *usb_dump_desc(char *start, char *end, struct usb_device *dev, int dump_strings)
{
	int i;
	...
	for (chix = 0; chix < usbdev->maxchild; chix++) {
		if (usbdev->children[chix]) {
			ret = usb_device_dump(buffer, nbytes, skip_bytes, file_offset, usbdev->children[chix],
					bus, level + 1, chix, ++cnt, dump_strings);
			if (ret == -EFAULT)
				return total_written;
			total_written += ret;
		}
...
static ssize_t usb_device_read_full(struct file *file, char *buf, size_t nbytes, loff_t *ppos, int dump_strings)
{
...
{
	return usb_device_read_full(file, buf, nbytes, ppos, 1);
}

static ssize_t usb_device_read_short(struct file *file, char *buf, size_t nbytes, loff_t *ppos)
{
	return usb_device_read_full(file, buf, nbytes, ppos, 0);
}
...
struct file_operations usbdevfs_devices_short_fops = {
	llseek:		usb_device_lseek,
	read:		usb_device_read_short,
	poll:		usb_device_poll,
	open:		usb_device_open,
	release:	usb_device_release,
};

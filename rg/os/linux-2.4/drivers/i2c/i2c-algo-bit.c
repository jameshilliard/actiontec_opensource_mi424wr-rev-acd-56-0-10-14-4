...
/* $Id: i2c-algo-bit.c,v 1.1.1.1 2006/01/20 03:16:11 zjjiang Exp $ */
...
static void i2c_start(struct i2c_algo_bit_data *adap) 
{
	...
	sdalo(adap);
	scllo(adap);
	udelay(40); /* add udelay to reset bus */
}
...
static int i2c_outb(struct i2c_adapter *i2c_adap, char c)
{
	...
	/* assert: scl is low */
	DEB2(printk(" i2c_outb:%2.2X udelay=%d\n",c&0xff, adap->udelay));
	for ( i=7 ; i>=0 ; i-- ) {
		sb = c & ( 1 << i );
		setsda(adap,sb);
		udelay(adap->udelay);
	...
	sdalo(adap);   /* yair: replaced sdahi with sdalo */
...
static int i2c_inb(struct i2c_adapter *i2c_adap) 
{
	...
	/* sdahi(adap);   yair: removed set of data to high */
		    ...
		    struct i2c_msg msgs[], int num)
{
	...
	DEB2(printk("i2c-algo-bit.o: bit_xfer num=%d\n", num));

	i2c_start(adap);
	for (i=0;i<num;i++) {
		pmsg = &msgs[i];
		DEB2(printk("i2c-algo-bit.o: bit_xfer pmsg[%d]->len=%d\n", i, pmsg->len));

 ...
 *
 * $Id: mtdblock.c,v 1.1.1.1 2006/01/20 03:16:02 zjjiang Exp $
 *
		      ...
		      unsigned int cmd, unsigned long arg)
{
	...
	case MEMLOCK:
	{
		...
		if (copy_from_user(adrs ,(void *)arg, sizeof(adrs)))
			return -EFAULT;

		if (!mtd->lock)
			return -EOPNOTSUPP;
		else
			return mtd->lock(mtd, adrs[0], adrs[1]);
	}
	...
	case MEMUNLOCK:
	{
		...
		if (!mtd->unlock)
			...
			return mtd->unlock(mtd, adrs[0], adrs[1]);
	}

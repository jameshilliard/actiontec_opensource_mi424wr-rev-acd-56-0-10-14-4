...
MODULE_DESCRIPTION ("MARS Cipher / CryptoAPI");
#endif
#ifdef MODULE_AUTHOR
MODULE_AUTHOR ("Dr Brian Gladman <brian.gladman@btinternet.com>");
#endif
	      ...
	      int atomic)
{
	...
	a = le32_to_cpu (in_blk[0]) + l_key[0];
	b = le32_to_cpu (in_blk[1]) + l_key[1];
	c = le32_to_cpu (in_blk[2]) + l_key[2];
	d = le32_to_cpu (in_blk[3]) + l_key[3];
	...
	out_blk[0] = cpu_to_le32 (a - l_key[36]);
	out_blk[1] = cpu_to_le32 (b - l_key[37]);
	out_blk[2] = cpu_to_le32 (c - l_key[38]);
	out_blk[3] = cpu_to_le32 (d - l_key[39]);
	      ...
	      int atomic)
{
	...
	d = le32_to_cpu (in_blk[0]) + l_key[36];
	c = le32_to_cpu (in_blk[1]) + l_key[37];
	b = le32_to_cpu (in_blk[2]) + l_key[38];
	a = le32_to_cpu (in_blk[3]) + l_key[39];
	...
	out_blk[0] = cpu_to_le32 (d - l_key[0]);
	out_blk[1] = cpu_to_le32 (c - l_key[1]);
	out_blk[2] = cpu_to_le32 (b - l_key[2]);
	out_blk[3] = cpu_to_le32 (a - l_key[3]);

	return 0;
}

#define CIPHER_ID                mars
#define CIPHER_BLOCKSIZE         128
#define CIPHER_KEY_SIZE_MASK     CIPHER_KEYSIZE_128 | CIPHER_KEYSIZE_192 | CIPHER_KEYSIZE_256
#define CIPHER_KEY_SCHEDULE_SIZE (40*sizeof(u32))

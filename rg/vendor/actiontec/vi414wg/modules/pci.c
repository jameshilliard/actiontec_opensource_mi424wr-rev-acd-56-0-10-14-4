/****************************************************************************
 *
 * rg/vendor/actiontec/vi414wg/modules/pci.c
 * 
...
void __init vi414wg_pci_hw_init(void)
{
    /* Disable PCI clock */
    *IXP425_GPIO_GPCLKR &= ~IXP425_PCI_CLK_ENABLE;
    ...
    vi414wg_latch_set(VI414WG_LATCH_PCI_RESET, 0);
    ...
    *IXP425_GPIO_GPCLKR |= IXP425_PCI_CLK_ENABLE;
    ...
    vi414wg_latch_set(VI414WG_LATCH_PCI_RESET, 1);
...
void __init vi414wg_pci_init(void *sysdata)
{
#ifdef CONFIG_PCI_RESET
...
static int __init vi414wg_map_irq(struct pci_dev *dev, u8 slot, u8 pin)
{
    static int irqs[3][2] = {
...
struct hw_pci vi414wg_pci __initdata = {
    .init = vi414wg_pci_init,
    .swizzle = common_swizzle,
    .map_irq = vi414wg_map_irq,
};

...
static void ieee802_1x_send(rtapd *rtapd, struct sta_info *sta, u8 type, u8 *data, size_t datalen)
{
	...
	DBGPRINT(RT_DEBUG_INFO,"Send to Sta with Identifier %d\n",*(data+1));
	memset(buf, 0, len);
	hdr3 = (struct ieee8023_hdr *) buf;
	memcpy(hdr3->dAddr, sta->addr, ETH_ALEN);
	memcpy(hdr3->sAddr, rtapd->own_addr, ETH_ALEN);
	...
	if (RT_ioctl(rtapd, CODE_SET, RT_OID_802_11_RADIUS_DATA, buf, len, sta->ApIdx) )
...
void ieee802_1x_tx_key(rtapd *rtapd, struct sta_info *sta, u8 id)
{
	...
	if (RT_ioctl(rtapd, CODE_SET, OID_802_11_ADD_KEY, (u8*)&WepKey, sizeof(NDIS_802_11_KEY), sta->ApIdx))
	{
		DBGPRINT(RT_DEBUG_ERROR," ioctl error : OID_802_11_ADD_KEY\n");
		return;
	}
...
static void ieee802_1x_encapsulate_radius(rtapd *rtapd, struct sta_info *sta, u8 *eap, size_t len)
{
	...
	snprintf(buf, sizeof(buf), RADIUS_802_1X_ADDR_FORMAT, MAC2STR(rtapd->own_addr));	
...
static void handle_eap(struct sta_info *sta, u8 *buf, size_t len)
{
	...
	DBGPRINT(RT_DEBUG_INFO," Receive EAP: code=%d identifier=%d length=%d from Supplicant\n",
...
void ieee802_1x_receive(rtapd *rtapd, u8 *sa, u8 *apidx, u8 *buf, size_t len)
{
	struct sta_info *sta;
	struct ieee802_1x_hdr *hdr;
	...
	switch (hdr->type)
	{
		...
		case IEEE802_1X_TYPE_EAPOL_START:
			DBGPRINT(RT_DEBUG_ERROR,"Handle EAPOL_START \n");
				...
				u8 *shared_secret, size_t shared_secret_len)
{
	...
	DBGPRINT(RT_DEBUG_TRACE, "IEEE802_1x_Get_Keys, PMK_len = %d\n",keys->recv_len );
	...
	if (RT_ioctl(rtapd, CODE_SET, OID_802_11_ADD_KEY, (u8*)&WepKey, sizeof(NDIS_802_11_KEY), sta->ApIdx))
	{
		DBGPRINT(RT_DEBUG_TRACE," ioctl error : OID_802_11_ADD_KEY\n");
		return;
	}

...
processPADI(Interface *ethif, PPPoEPacket *packet, int len)
{
    ...
    if (!NumServiceNames)
	ok = 1;			/* No Service-Name defined for PPPoE server */
    ...
    /* If no service-names specified on command-line, send the name requested
     * by the client.  Otherwise, add all service-name tags */
...
static void sigchild_disable(void)
{
    sigset_t set;
	...
	syslog(LOG_ERR, "Error setting masking for SIGCHLD signal");
}

static void sigchild_enable(void)
{
    sigset_t set;
...
processPADR(Interface *ethif, PPPoEPacket *packet, int len)
{
	...
	if (!NumServiceNames)
	    serviceName = requestedService.payload;
    ...
    /* Mask the sigchild until fork result is handled */
    sigchild_disable();
...
startPPPDUserMode(ClientSession *session)
{
    ...
    char serviceNameBuffer[SMALLBUF] = "";
    ...
    if (strlen(session->serviceName))
        snprintf(serviceNameBuffer, SMALLBUF, " -S '%s'", session->serviceName);

    /* Let's hope service-name does not have ' in it... */
    snprintf(buffer, SMALLBUF, "%s -n -I %s -e %d:%02x:%02x:%02x:%02x:%02x:%02x%s %s",

...
static int print_header(char *filename, int print_cr)
{
	const char *header1 = "\n==> ";
	const char *header2 = " <==\n";
	int res;

	res = full_write(STDOUT_FILENO, header1+!print_cr,
		strlen(header1)-!print_cr) <= 0;
	res |= full_write(STDOUT_FILENO, filename, strlen(filename)) <= 0;
	res |= full_write(STDOUT_FILENO, header2, strlen(header2)) <= 0;
...
int tail_main(int argc, char **argv)
{
	...
	for (i = 0; i < nfiles; i++) {
		...
		{
			if (print_header(argv[optind + i], i) < 0)
			{
				perror_msg("print_header");
				status = EXIT_FAILURE;
				break;
			}
		}

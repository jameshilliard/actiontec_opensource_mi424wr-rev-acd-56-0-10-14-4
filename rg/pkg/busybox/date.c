...
{
	int nr;
	char *dot = strchr(t_string, '.');

	// get seconds if exists
	if (dot)
	{
		*dot++ = 0;
		tm_time->tm_sec = atoi(dot);
	}
...
#define COPY_TM(x, y) memcpy(x, y, sizeof(*x))

	/* Parse input and assign appropriately to tm_time */

	if (COPY_TM(&t, tm_time),sscanf(t_string, "%d:%d:%d",
			   &t.tm_hour, &t.tm_min, &t.tm_sec) == 3) {
					/* no adjustments needed */

	} else if (COPY_TM(&t, tm_time),sscanf(t_string, "%d:%d",
					  &t.tm_hour, &t.tm_min) == 2) {
					/* no adjustments needed */


	} else if (COPY_TM(&t, tm_time),sscanf(t_string, "%d.%d-%d:%d:%d",
					  &t.tm_mon,
					  &t.tm_mday,
					  &t.tm_hour,
					  &t.tm_min, &t.tm_sec) == 5) {
	...
	} else if (COPY_TM(&t, tm_time),sscanf(t_string, "%d.%d-%d:%d",
					  &t.tm_mon,
					  &t.tm_mday,
	...
	} else if (COPY_TM(&t, tm_time),sscanf(t_string, "%d.%d.%d-%d:%d:%d",
					  &t.tm_year,
					  &t.tm_mon,
					  &t.tm_mday,
					  &t.tm_hour,

...
int copy_file_chunk(int srcfd, int dstfd, size_t chunksize)
{
        ...
        while (chunksize > 0) {
                if (chunksize > BUFSIZ)
                        ...
                        size = chunksize;
                if (full_write(dstfd, buffer, full_read(srcfd, buffer, size)) < (int)size)
                        return FALSE;
                chunksize -= size;
        }
        return TRUE;
...
extern char * xstrdup (const char *s) {
	...
	t = xcalloc(strlen(s)+1,sizeof(char));
	if (t == NULL)
		error_msg_and_die(memory_exhausted);
...
char process_escape_sequence(char **ptr)
{
       ...
       if (num_digits == 0) {	/* mnemonic escape sequence? */
		   ...
		   if (*p)
			   ...
			   n = *q++; /*return the first character after '\'*/
	   }
...
{
	const char *name = x;
	const struct BB_applet *applet = (struct BB_applet *)y;
...
{
	return (struct BB_applet *)bsearch(name, (char *)applets, NUM_APPLETS, sizeof(struct BB_applet),
			applet_name_compare);
}
...
const char *make_human_readable_str(unsigned long val, unsigned long hr)
{
	...
	if(val == 0)
		return("0");
	if(hr)
		snprintf(str, 10, "%lu", val/hr);
	else {
		while(val >= divisor && i <= 4 && (i+1)*10 <= 8*sizeof(divisor)) {
			divisor=divisor<<10, i++;
		} 
		if (divisor >= val)
		    divisor=divisor>>10, i--;
		/* if no divisor, display the size with no floating point
		 * and no postfix (since it is in bytes)
		 */
		if (divisor<=1)
		    snprintf(str, 10, "%lu", val);
		else {
		    snprintf(str, 10, "%.1Lf%c", (long double)(val)/divisor,
			    strings[i-1]);
		}
	}
	return(str);
...
void trim(char *s)
{
	...
	ptrend = s + len - 1;
	while(ptrend >= s && *ptrend && isspace(*ptrend))
		*ptrend-- = '\0';

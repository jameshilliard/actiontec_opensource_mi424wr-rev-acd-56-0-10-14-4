...
#ifdef CONFIG_INIT_BASIC_RUNLEVELS
int runlevel = 3;
#endif
...
#define INITTAB      "/inittab"  /* inittab file location */
#ifndef INIT_SCRIPT
#ifdef CONFIG_RG_SYSINIT
#define INIT_SCRIPT  "/bin/rg_sysinit"   /* Default sysinit script. */
#else
#define INIT_SCRIPT  "/bin/simple_init.sh"
#endif
#endif
...
#if defined(CONFIG_ARCH_IXP22X) || defined (CONFIG_MPC8272ADS)
static char *secondConsole = NULL;
static char *thirdConsole  = NULL;
static char *fourthConsole = NULL;
#else
...
static void parse_inittab(void)
{
	...
	while (fgets(buf, 255, file) != NULL) {
		...
		/* Check runlevel */
#ifdef CONFIG_INIT_BASIC_RUNLEVELS
		if (*runlev && !strchr(runlev, '0' + runlevel))
			continue;
#endif
...
#define STR_SIZE 5
        ...
        if (fp)
        {
            ...
            FD_ZERO(&rfds);
            FD_SET(fd, &rfds);
            message(CONSOLE, "Press y and ENTER to stop at runlevel 1\n");

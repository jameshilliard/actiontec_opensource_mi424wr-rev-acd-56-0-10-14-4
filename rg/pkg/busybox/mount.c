...
{
	static char string_flags_buf[1024];
	char *string_flags;
	char *extra_opts;
	int flags = 0;
	...
	string_flags_buf[0] = 0;
	string_flags = extra_opts = string_flags_buf;
...
singlemount:			
			...
			if (strcmp(filesystemType, "nfs") == 0) {
				if (nfsmount(device, get_dir_path(directory), &flags, 
...
#endif
			if (!mount_one(device, get_dir_path(directory), filesystemType,
				flags, string_flags, useMtab, fakeIt, extra_opts, TRUE))
				rc = EXIT_FAILURE;
...
char *get_dir_path(char *directory)
{

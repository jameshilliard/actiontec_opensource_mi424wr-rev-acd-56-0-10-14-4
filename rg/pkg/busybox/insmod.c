...
int new_sys_init_module(const char *name, const struct module *);
...
int obj_relocate (struct obj_file *f, ElfW(Addr) base, char *m_name);

struct obj_file *obj_load(FILE *f);

int obj_create_image (struct obj_file *f, char *image);

void free_obj(struct obj_file *f);
...
static int obj_load_order_prio(struct obj_section *a)
{
	...
	ac = 0;
#ifdef CONFIG_ARCH_IXP22X
	if (strncmp(a->name, ".fast_ram_", 10))
	{
		ac |= 64; /* we want regular sections to be placed before fast ones */
		a->is_fast = 0;		
	}
	else
		a->is_fast = 1;
...
static int new_create_module_ksymtab(struct obj_file *f)
{
	...
	if (flag_export && !obj_find_section(f, "__ksymtab")) {
		...
		loaded = xmalloc(sizeof(int) * (i = f->header.e_shnum));
		while (--i >= 0)
...
new_init_module(const char *m_name, struct obj_file *f,
				unsigned long m_size, void *image)
{
...
#ifdef CONFIG_ARCH_IXP22X
	module->fast_mem_size = f->fast_mem_size;
	module->fast_mem = (void *)f->fastaddr;
#endif

	sec = obj_find_section(f, "__ksymtab");
	if (sec && sec->header.sh_size) {
		module->syms = (struct module_symbol *)sec->header.sh_addr;
		module->nsyms = sec->header.sh_size / (2 * tgt_sizeof_char_p);
	}

	if (n_ext_modules_used) {
		sec = obj_find_section(f, ".kmodtab");
		module->deps = (struct module_ref *)sec->header.sh_addr;
		module->ndeps = n_ext_modules_used;
	}

	module->init =
		(int (*)(void))obj_symbol_final_value(f, obj_find_symbol(f, "init_module"));
	module->cleanup =
		(void (*)(void))obj_symbol_final_value(f, obj_find_symbol(f, "cleanup_module"));

	sec = obj_find_section(f, "__ex_table");
	if (sec) {
		module->ex_table_start = (struct exception_table_entry *)sec->header.sh_addr;
		module->ex_table_end = (struct exception_table_entry *)(sec->header.sh_addr + sec->header.sh_size);
	}
	...
	ret = new_sys_init_module(m_name, (struct module *) image);
	if (ret)
		perror_msg("init_module: %s", m_name);
...
unsigned long obj_load_size(struct obj_file *f)
{
	...
	for (sec = f->load_order; sec; sec = sec->load_next) {
...
#ifdef CONFIG_ARCH_IXP22X
		if (sec->is_fast && align && (fast_dot & (align - 1)))
			fast_dot = (fast_dot | (align - 1)) + 1;		
#endif
		...
		if (sec->is_fast)
		{
			sec->proxy_addr = sec->header.sh_addr;
			sec->header.sh_addr = fast_dot;
			fast_dot += sec->header.sh_size;
		}
#endif
	}

#ifdef CONFIG_ARCH_IXP22X
	f->fast_mem_size = fast_dot;
	dot -= fast_dot;
#endif
...
int obj_relocate(struct obj_file *f, ElfW(Addr) base, char *m_name)
{
	...
	{
#ifdef CONFIG_ARCH_IXP22X
		if (f->sections[i]->is_fast)
		{
			f->sections[i]->header.sh_addr += f->fastaddr;
			f->sections[i]->proxy_addr += base;
		}
		else
#endif
...
int obj_create_image(struct obj_file *f, char *image)
{
	...
	if (mem_fd<0)
	{
		printf("insmod: error opening %s. resorting to default insmod "
			"behaviour\n", KMEM_DEV);
	}
	else
	{
	...
	for (sec = f->load_order; sec; sec = sec->load_next) {
...
#ifdef CONFIG_ARCH_IXP22X
		/* if we write directly to the kernel memory, the section address is 
		 * in sec->header.sh_addr.
		 * sec->proxy_addr is no longer needed because the kernel does not need
		 * to relocate this section from the ram to the fast ram.
		 */
		if (mem_fd<0 && sec->is_fast)
			secimg = image + (sec->proxy_addr - base);
		else
#endif
		...
		else
		{
			...
			lseek(mem_fd, (int)secimg + offset, SEEK_SET);
...
struct obj_file *obj_load(FILE * fp)
{
	...
	fseek(fp, 0, SEEK_SET);
	if (fread((char *)&f->header, sizeof(f->header), 1, fp) != 1) {
	...
	section_headers = xmalloc(sizeof(ElfW(Shdr)) * shnum);
	fseek(fp, f->header.e_shoff, SEEK_SET);
	if (fread((char *)section_headers, sizeof(ElfW(Shdr)), shnum, fp) != shnum) {
...
void free_obj(struct obj_file *f)
{
	...
	for (i=0; i<f->header.e_shnum; i++)
	{
		free(f->sections[i]->contents);
		free(f->sections[i]);
	}
	free(f->sections);
	free(f->string_patches);
	free(f->symbol_patches);
	for (i=0; i<HASH_BUCKETS; i++)
	{
		struct obj_symbol *p = f->symtab[i], *tmp;

		while (p)
		{
			for (j=0; j<f->local_symtab_size; j++)
				if (p==f->local_symtab[j])
				{
					f->local_symtab[j] = NULL;
					break;
				}
			tmp = p->next;
			free(p);
	...
	for (i=0; i<f->local_symtab_size; i++)
		free(f->local_symtab[i]);
	free(f->local_symtab);
	free(f);
}
...
extern int insmod_main( int argc, char **argv)
{
	...
	/* Parse any options */
	while ((opt = getopt(argc, argv, "fktvxLo:")) > 0) {
		switch (opt) {
			...
			case 'k':			/* module loaded by kerneld, auto-cleanable */
				flag_autoclean = 1;
				break;
			case 't':			/* use permanent allocation */
				flag_permanent = 1;
				break;
	...
	if (flag_permanent)
	{
		memmove(m_name + 10, m_name, len);
		memcpy(m_name, "permanent_", 10);
	}
	m_addr = create_module(m_name, m_size);
	if (flag_permanent)
		memmove(m_name, m_name + 10, len);
#ifdef CONFIG_ARCH_IXP22X
	if (m_addr != -1 && f->need_fast_ram)
	{
		char *f_name = (char *)malloc(strlen(m_name) + 6);
		memcpy(f_name, "sram_", 5);
		strcpy(f_name + 5, m_name);
		
	    f->fastaddr = create_module(f_name, f->fast_mem_size);
	...
	if (k_new_syscalls 
		? !new_init_module(m_name, f, m_size, (void *)m_addr)

...
#ifdef BB_FEATURE_GETUSERNAME_AND_HOMEDIR
static char *user_buf = NULL;
static char *home_pwd_buf = NULL;
static int my_euid;
#endif
...
static void cmdedit_free(void)
{
	if (his_front) {
		...
		free(his_end->s);
		free(his_end);
		his_front = his_end = NULL;
		history_counter = 0;
	}
	free(user_buf);
	free(home_pwd_buf);
	home_pwd_buf = user_buf = NULL;
	free(cmdedit_prompt);
}
#endif

static void cmdedit_atexit(void)
{
#ifdef BB_FEATURE_CLEAN_UP
	cmdedit_free();
#endif
...
#endif	
			case '!':
				snprintf(buf, MAX_PRMPT_ADD, "%d", num_ok_lines);
...
extern void cmdedit_init(void)
{
	...
	if ((handlers_sets & SET_ATEXIT) == 0) {
		...
		atexit(cmdedit_atexit);	/* be sure to do this only once */
	}
...
#define collapse_pos(is, in) { \
	memmove(int_buf+(is), int_buf+(in), (len-(in))*sizeof(int)); \
	memmove(pos_buf+(is), pos_buf+(in), (len-(in))*sizeof(int)); }
...
static int find_match(char *matchBuf, int *len_with_quotes)
{
	...
	int command_mode;
	int c, c2;
	int len = strlen(matchBuf)+1;
	int *int_buf = xmalloc(len*sizeof(int));
	int *pos_buf = xmalloc(len*sizeof(int));
...
static void input_tab(int *lastWasTab)
{
	...
	if (*lastWasTab == FALSE) {
		...
		*lastWasTab = TRUE;		/* flop trigger */
		if (cursor)
			for (;command_ps[cursor - count_bs - 1] == '\\'; count_bs++);
		if (count_bs % 2)
		{
			input_backspace();
			command_ps[cursor] = 0;
...
extern void cmdedit_read_input(char *prompt, char command[BUFSIZ])
{
	...
	while (1) {
		...
		do
		{
			...
			if (r < 1 && errno != EINTR)
				//return;
				clean_up_and_die(0);/* if we can't read input then exit */
		} while (r < 1);
